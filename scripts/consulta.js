...
async function searchInfo(){
    ...
    const response = await fetch('https://home.gruponobre.edu.br/.netlify/functions/search_school_student', {
        method: 'post',
        body: JSON.stringify({
            cpf: reqCPF,
    ...
    if(response.status == 200){
        ...
        nome.innerHTML = `<img class="idIcon" src="./img/rosto.png" alt="rosto" width="60px"/>${data.resposta.nome}`
        registro.innerHTML = `<img class="idIcon" src="./img/cracha.png" alt="cracha" width="60px"/>${data.resposta.ra}`
    }else{
